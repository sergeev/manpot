...
class Database {
    ...
    function NewConnection(){ 
		...
		if(!$this->connection)
			$this->error("Невозможно присоединится к серверу : ".$this->host."");
      
		if(!@mysql_select_db($this->database, $this->connection))
			$this->error("Невозможно присоединится к базе данных : ".$this->database);
    ...
    function query($sql){
		...
		if(!$this->queryid)
			$this->error("Невозможно выполнить запрос: ".$sql);
    ...
    function paginate($query){ 
       ...
       //if($pages==null) $pages = 1;       
		$links = "<table width='100%' border='0' cellpadding='0' cellspacing='0'><tr><td class='rankingHeader'><b>Страница:</b> ";
    ...
    function error($str="") {
        ...
        ?>
			<table width="50%" align="center" cellspacing="2" style='border: 1px dashed #efefef; background-color: #FFFFFF; color: #000000; font-size: 12px;' cellpadding="2">
				...
				<tr>
					<td><b>Ошибка MySQL (#<? echo $this->errorno;?>):</b></td>
