...
<div class="clear"></div>
    <div align="center">
        ...
        <h1>Установка MaNPOT...</h1>
        
        <?php
        if($_POST){
            $mydb->query_insert("users", array('id' => null, 'username' => $_POST[username], 'password' => md5($_POST[password]), 'email' => $_POST[email], 'acl' => 0));
            $main->message("Аккаунт Администратора Создан");
            
            echo "<h2 style='color: red;'>Удалите этот файл во избежание создания аккаунта администратора другими пользователями.</h2>";
        }else{
...
if(strlen($mydb->errorno) == 0)
echo "<b>Project импортирована....</b><br/>";
...
$mydb->query($userSQL);
if(strlen($mydb->errorno) == 0)
echo "<b>User импортирована....</b><br/>";
...
$mydb->query($todoSQL);
if(strlen($mydb->errorno) == 0)
echo "<b>ToDo_main импортирована....</b><br/>";
...
$mydb->query($todoLSQL);
if(strlen($mydb->errorno) == 0)
echo "<b>ToDo_List импортирована....</b><br/>";
...
$mydb->query("ALTER TABLE `list` ADD `attachment` VARCHAR( 100 ) NOT NULL ;");
        ...
        <table width="300" cellspacing="2" cellpadding="2" border="0">
            ...
            <tr>
                 <td colspan="2" align="center"><input type="submit" name="finish" value="Закончить Установку"></td>
